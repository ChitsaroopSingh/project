...
def main(page, search, category):
    product_rating = []
    product_brand = []
    product = []
    driver.get(f"{PRODUCT_SITE}{search}?f=Categories:{category}&p={page}")
...
if __name__ == "__main__":
    ...
    pages = DEFAULT_PAGES
    search = SEARCH_TEXT
    category_list = [x for x in CATEGORIES if CATEGORY in x]
    category_str = ','.join(category_list)
    for page in range(1, pages+1):
        product_brand, product, product_rating = main(
            page, search, category_str)
